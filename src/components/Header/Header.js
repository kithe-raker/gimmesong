    ...
    <header className="gimmesong-bg fixed top-0 right-0 left-0 z-50 mx-auto flex h-[60px] w-full max-w-md items-center justify-between px-2.5">
      ...
      {user ? (
          ...
          {user?.username && (
            <>
              ...
              <button
                ...
                className={`group mr-1.5 flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-full bg-white shadow-sm ${
                  location.pathname === "/request" ? "bg-black" : ""
                }`}
              >
                <svg
                  className={`${
        ...
        location.pathname !== "/" && (
          <button
            onClick={() => navigate("/")}
            className="flex h-10 items-center rounded-full bg-gradient-to-r from-[#86C7DF] via-[#8583D6] to-[#CFB6D0] px-3 text-sm text-white transition duration-150 ease-in-out"
          >
            <svg
